...
(defmacro delayed-seq
  "Similar to lazy-seq, but returns a seq whose body is not memoized
  after traversal. This gives you more options when working within an
  environment that retains the head, but at the cost of losing
  repeatability and should be forced only once if backed by a
  side-effectful iterator."
  {:added "1.4"}
  [& body]
  (list 'new 'clojure.lang.DelayedSeq (list* fn* [] body)))
